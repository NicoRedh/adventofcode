...
public class SquidWin {
    ...
    public static void main(String[] args) {
        ...
        List<Board> lastWinner = new ArrayList<Board>();
        List<Integer> lastNum = new ArrayList<Integer>();
        for (Integer nums : numbers) {
            for (int i = 0; i<boards.size(); i++) {
                if (boards.get(i).check(nums) && !lastWinner.contains(boards.get(i))) {
                    lastWinner.add(boards.get(i));
                    lastNum.add(nums);
                    if(lastWinner.size()>99)System.out.println(lastWinner.get(lastWinner.size()-1).winner(lastNum.get(lastNum.size()-1)));
